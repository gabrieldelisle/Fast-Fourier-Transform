...
int main(int argc, char **argv) {
  ...
  origin =
      (int *)malloc(L * sizeof(int)); // array for receiving the previous one.
  int *origin2;
  ...
  MPI_Alltoall(&X[0], L / P, MPI_DOUBLE_COMPLEX, &X[L], L / P,
               MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD);
  MPI_Alltoall(&origin[0], L / P, MPI_INT, &origin2[0], L / P, MPI_INT,
               MPI_COMM_WORLD);
  ...
  for (i = 1; i < logN + 1; ++i) {
    ...
    if (m / 2 >= L) {
      ...
      if (p > p_other) {
        MPI_Recv(&X[L], L, MPI_DOUBLE_COMPLEX, p_other, tag, MPI_COMM_WORLD,
                 &status);
